            ...
            if adjustFinalCellOrderIfNecessary() {
                ...
                UIView.animate(withDuration: 0.2,
                                           ...
                                           completion: { (completed) in
                                            ...
                                            print(location)
                                            var newLocation: CGPoint = location
                                            if location.y < 30
                                            {
                                                newLocation = CGPoint.init(x: newLocation.x, y: 30)
                                            }
											if let currentIndexPath = self.indexPathForRow(at: newLocation) {
												self.dragableDelegate?.tableView?(self, endDragCellTo: currentIndexPath)
											}
                                            else
                                            {
//                                                if location.y > self.frame.size.height + 60
//                                                {
//                                                    
//                                                }
                                                let lastRow = self.numberOfRows(inSection: 1)
                                                let currentIndexPath1 = IndexPath.init(row: lastRow - 1, section: 1)
                                                self.dragableDelegate?.tableView?(self, endDragCellTo: currentIndexPath1)
                                            }
                })
            }
        }
    ...
    func adjustCellOrderIfNecessary(){
        ...
        NSLog("dragingIndexPath section : %ld", dragingIndexPath.section)
        NSLog("dragingIndexPath: %ld", dragingIndexPath.row)
        
        NSLog("toIndexPath section : %ld", toIndexPath.section)  
        NSLog("toIndexPath Row: %ld", toIndexPath.row)
